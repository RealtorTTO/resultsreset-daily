...
async def generate_plan(request: Request):
    try:
        ...
        # Fix 3: Log raw plan_text for debugging
        logger.info(f"[GENERATE] plan_text start: {plan_text[:500]!r}")
        logger.info(f"[GENERATE] plan_text end: {plan_text[-500:]!r}")

        # Fix 2: Guard against truncated response
        if message.stop_reason == "max_tokens":
            logger.error(f"[GENERATE] plan truncated at max_tokens for agent={agent_name!r}")
            return JSONResponse(
                status_code=422,
                content={
                    "error": "plan_truncated",
                    "message": "The plan generation was cut short. Please try again — if this keeps happening, contact Teresa."
                }
            )
